...
fn test_onchain() {
    ...
    t.execute_with(|| {
        ...
        assert_ok!(OffchainDemo::save_number(
                Origin::signed(acct),
                num
        ));
        ...
        assert!(System::events()
            .iter()
            .any(|er| er.event == TestEvent::offchain_demo(RawEvent::NumberSaved(Some(acct), num))));
...
fn test_offchain() {
    ...
    t.execute_with(|| {
        // when
        let num: u32 = 5;
        OffchainDemo::signed_submit_number(num.into()).unwrap();
        // then
        ...
        let mut b_number = num +1;
        let mut total = 0;
        while b_number != 0 {
            total = total + b_number * b_number;
            b_number = b_number - 1;
        }
        assert_eq!(tx.call, Call::save_number(total));
    });
}
